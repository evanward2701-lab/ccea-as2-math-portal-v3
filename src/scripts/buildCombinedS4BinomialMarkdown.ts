...
const SOURCE_DIR =
  '/Users/evanward/Documents/AS Portal - New /AS2 Files/Practice Questions-S4-Binomial Distribution';
const PACK_PREFIX = 'AS2_binomial_distribution_probability';
...
function renderAppendixVisuals(visualRegistry: Map<string, VisualBlock>, usedVisualIds: Set<string>): string[] {
  const appendixVisuals = [...visualRegistry.values()].filter((visual) => !usedVisualIds.has(visual.id));
  ...
  return [
    '# Appendix: Support Visuals and Interactive Checking Tool',
    '',
...
function extractUnmatchedMarkSchemeSection(content: string): string | null {
  const match = content.match(/^## (?:\d+\.\s+)?Unmatched Mark Scheme Entries\n([\s\S]*)/mu);
...
function buildCombinedMarkdown(): void {
  ...
  const chunks: string[] = [
    '# AS2 Binomial Distribution Probability Combined Study Pack',
    '',
