...
const SOURCE_DIR = '/Users/evanward/Documents/AS Portal - New /AS2 Files/Practice Questions-S2-Correlation';
const PACK_PREFIX = 'AS2_product_moment_correlation_coefficient';
...
function parseSolutionSections(content: string): NumberedSection[] {
  const sections: NumberedSection[] = [];
  const regex =
    /^## Solution to Question\s+(\d+):\s*(.*?)\n([\s\S]*?)(?=^## Solution to Question\s+\d+:|^## \d+\. Unmatched Mark Scheme Entries|^## Unmatched Mark Scheme Entries|(?![\s\S]))/gmu;
...
function buildCombinedMarkdown(): void {
  ...
  const chunks: string[] = [
    '# AS2 Product-Moment Correlation Coefficient Combined Study Pack',
    '',
