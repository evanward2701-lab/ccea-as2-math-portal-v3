...
export function PracticeBank() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto w-full flex flex-col gap-8 bg-background text-foreground">
      {/* Configuration Filter Sidebar */}
      <aside className="w-full border border-primary/10 bg-card p-6 rounded-2xl shadow-xl backdrop-blur-sm">
        <h2 className="text-xs uppercase tracking-widest text-primary font-mono font-bold mb-6">Filter Matrix</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label className="block text-2.5 uppercase font-mono tracking-wider font-bold text-muted-foreground mb-2">Module Topic</label>
            <select 
              value={searchParams.get("module") || "All"} 
              onChange={(e) => updateFilter("module", e.target.value)}
              className="w-full text-xs font-mono bg-background border border-primary/20 text-foreground rounded-lg p-2.5 outline-none transition-colors"
            >
          ...
          <div>
            <label className="block text-2.5 uppercase font-mono tracking-wider font-bold text-muted-foreground mb-2">Syllabus Type</label>
            <select 
              value={selectedType} 
          ...
          <div>
            <label className="block text-2.5 uppercase font-mono tracking-wider font-bold text-muted-foreground mb-2">Mark Allocation</label>
              ...
              {["All", "1-3", "4-6", "7+"].map((tier) => (
                <button
                  ...
                  className={`w-full text-left text-xs font-mono px-3 py-2 rounded-lg transition-all border ${
                    selectedMarks === tier
                      ? "bg-primary/10 border-primary/30 text-primary font-bold shadow-inner"
      ...
      <main className="flex-1 flex flex-col">
        <div className="flex items-center justify-between border-b border-primary/10 pb-4 mb-4">
          <div className="text-xs font-mono text-muted-foreground">
            Yield Output: <span className="text-primary font-bold">{filteredQuestions.length}</span> items matching criteria
          </div>
        </div>

        {filteredQuestions.length === 0 ? (
          <div className="p-12 text-center border border-dashed border-primary/20 rounded-2xl text-muted-foreground font-serif italic text-sm bg-muted/30 mt-8">
