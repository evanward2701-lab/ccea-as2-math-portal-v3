      ...
      <div className="w-full rounded-2xl border border-primary/20 bg-card p-6 text-foreground">
        <div className="grid gap-4 md:grid-cols-3">
          <label className="flex flex-col gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            ...
            <span className="text-lg font-mono text-foreground">{base}</span>
          </label>
          <label className="flex flex-col gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            Numerator m
            ...
            <span className="text-lg font-mono text-foreground">{numerator}</span>
          </label>
          <label className="flex flex-col gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            Denominator n
            ...
            <span className="text-lg font-mono text-foreground">{safeDenominator}</span>
          </label>
        </div>
        <div className="mt-6 grid gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4 font-mono text-sm">
          <div className="text-muted-foreground">a^(m/n) = <span className="text-foreground">{fmt(value)}</span></div>
          <div className="text-muted-foreground">Root form: <span className="text-foreground">{rootForm}</span></div>
          <div className="text-muted-foreground">Repeated root form: <span className="text-foreground">{repeatedRootForm}</span></div>
        </div>
      </div>
    </DiagramPanel>
  );
