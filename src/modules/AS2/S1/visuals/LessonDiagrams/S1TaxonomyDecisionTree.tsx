    ...
    >
      <div className="w-full max-w-2xl flex flex-col items-center">
        <div className="mb-6 text-center">
          <p className="text-sm text-zinc-400 uppercase tracking-wider">Variable to Classify:</p>
          <p className="text-lg font-bold text-zinc-300 mt-1">"{currentExample.label}"</p>
        </div>

        <div className="space-y-4 w-full">
          <div className={cn("p-4 rounded-lg border bg-zinc-900/50", step > 0 ? "border-zinc-800" : "border-zinc-500/50")}>
            <p className="text-sm font-bold text-zinc-300 mb-3">1. Is the data numerical or descriptive?</p>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => handleDecision('numerical')} disabled={step > 0} className={cn("p-3 text-sm font-semibold rounded-md transition-all", step === 0 && "bg-zinc-800 hover:bg-zinc-700 text-zinc-200", step > 0 && userPath[0] === 'numerical' && "bg-emerald-950/50 border border-emerald-700 text-emerald-300", step > 0 && userPath[0] !== 'numerical' && "bg-zinc-800/50 text-zinc-500 opacity-50")}>Numerical</button>
              <button onClick={() => handleDecision('categorical')} disabled={step > 0} className={cn("p-3 text-sm font-semibold rounded-md transition-all", step === 0 && "bg-zinc-800 hover:bg-zinc-700 text-zinc-200", step > 0 && userPath[0] === 'categorical' && "bg-emerald-950/50 border border-emerald-700 text-emerald-300", step > 0 && userPath[0] !== 'categorical' && "bg-zinc-800/50 text-zinc-500 opacity-50")}>Categorical</button>
            </div>
          </div>

          {step > 0 && currentExample.path[0] === 'numerical' && (
            <div className={cn("p-4 rounded-lg border bg-zinc-900/50", step > 1 ? "border-zinc-800" : "border-zinc-500/50")}>
              <p className="text-sm font-bold text-zinc-300 mb-3">2. Is it counted or measured?</p>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => handleDecision('counted')} disabled={step > 1} className={cn("p-3 text-sm font-semibold rounded-md transition-all", step === 1 && "bg-zinc-800 hover:bg-zinc-700 text-zinc-200", step > 1 && userPath[1] === 'counted' && "bg-emerald-950/50 border border-emerald-700 text-emerald-300", step > 1 && userPath[1] !== 'counted' && "bg-zinc-800/50 text-zinc-500 opacity-50")}>Counted</button>
                <button onClick={() => handleDecision('measured')} disabled={step > 1} className={cn("p-3 text-sm font-semibold rounded-md transition-all", step === 1 && "bg-zinc-800 hover:bg-zinc-700 text-zinc-200", step > 1 && userPath[1] === 'measured' && "bg-emerald-950/50 border border-emerald-700 text-emerald-300", step > 1 && userPath[1] !== 'measured' && "bg-zinc-800/50 text-zinc-500 opacity-50")}>Measured</button>
              </div>
            </div>
          )}
          ...
          <div className="h-12 flex items-center justify-center">
            ...
            {isComplete && (
              <div className="text-center p-4 bg-emerald-950/50 border-2 border-emerald-600 rounded-lg w-full">
                ...
                <p className="text-xs text-zinc-400 mt-2 italic">{currentExample.explanation}</p>
              </div>
            )}
          </div>
        </div>
