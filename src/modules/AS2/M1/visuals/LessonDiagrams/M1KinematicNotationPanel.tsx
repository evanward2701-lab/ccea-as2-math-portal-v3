...
const notationCards: NotationCard[] = [
  { symbol: '$s$', label: 'Displacement', unit: '$\\text{m}$', tone: 'zinc' },
  { symbol: '$u$', label: 'Initial Velocity', unit: '$\\text{m s}^{-1}$', tone: 'zinc' },
  { symbol: '$v$', label: 'Final Velocity', unit: '$\\text{m s}^{-1}$', tone: 'zinc' },
...
const toneClasses: Record<NotationTone, { text: string; border: string; bg: string; unit: string }> = {
  emerald: {
    ...
    bg: 'bg-primary/12',
    unit: 'border-primary/25 bg-primary/15 text-primary',
  },
  amber: {
    text: 'text-primary',
    border: 'border-primary/25',
  ...
  zinc: {
    ...
    bg: 'bg-primary/5',
    unit: 'border-primary/10 bg-card text-foreground',
  },
};
...
const NotationCard: React.FC<NotationCard> = ({ symbol, label, unit, tone }) => {
  ...
  return (
    <div className={cn('flex min-h-36 flex-col items-center justify-center gap-3 rounded-xl border px-3 py-4 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.035),0_16px_34px_rgba(0,0,0,0.2)]', styles.border, styles.bg)}>
      ...
      <div className="flex min-h-10 items-center justify-center text-center text-[11px] font-black uppercase leading-tight text-muted-foreground text-balance break-words">
        {label}
      </div>
  ...
  return (
    <DiagramPanel
      ...
      analysis={
        <div className="mx-auto grid w-full max-w-5xl gap-4 px-2 md:grid-cols-2">
          <div className="flex items-center justify-center rounded-xl border border-primary/20 bg-primary/5 p-4 text-center text-sm leading-relaxed text-muted-foreground">
            <span className="mr-1.5 font-bold text-foreground">Sign Convention Rule:</span>
            Choose one positive direction before assigning vector signs.
          </div>
          <div className="flex items-center justify-center rounded-xl border border-destructive/20 bg-destructive/10 p-4 text-center text-sm font-medium leading-relaxed text-destructive">
            <span className="inline-flex flex-wrap items-baseline justify-center gap-1.5">
              <span className="font-bold text-destructive">Common Trap:</span>
              <span className="inline-flex items-baseline text-base font-bold leading-none text-destructive">
    ...
    >
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
        <div className="relative min-h-80 w-full overflow-hidden rounded-2xl border border-primary/20 bg-card shadow-2xl aspect-[16/8]">
          <SVGLibrary />
          <svg
            viewBox="0 0 800 400"
          ...
          >
            <defs>
              <linearGradient id="kinematic-axis-fade" x1="70" y1="0" x2="730" y2="0" gradientUnits="userSpaceOnUse">
                <stop stopColor="var(--border)" stopOpacity="0" />
                <stop offset="0.15" stopColor="var(--border)" stopOpacity="1" />
                <stop offset="0.85" stopColor="var(--border)" stopOpacity="1" />
                <stop offset="1" stopColor="var(--border)" stopOpacity="0" />
              </linearGradient>
            </defs>

            <rect x="42" y="42" width="716" height="300" rx="20" fill="var(--muted)" stroke="var(--border)" strokeWidth="1.2" opacity="0.78" />
            ...
            <line x1="160" y1="192" x2="160" y2="252" stroke="var(--muted-foreground)" strokeWidth="2" opacity="0.82" />
            <line x1="360" y1="192" x2="360" y2="252" stroke="var(--foreground)" strokeWidth="2" opacity="0.88" />
            <line x1="600" y1="192" x2="600" y2="252" stroke="var(--foreground)" strokeWidth="2" opacity="0.88" />
            <Particle cx={160} cy={222} r={6} fill="var(--card)" stroke="var(--foreground)" strokeWidth={2.6} />
            <Particle cx={360} cy={222} r={5.5} fill="var(--muted-foreground)" />
            <Particle cx={600} cy={222} r={5.5} fill="var(--muted-foreground)" />

            <rect x="418" y="112" width="138" height="40" rx="9" fill="var(--background)" stroke="var(--border)" strokeWidth="1.5" />
          </svg>

          <div className="absolute inset-0 z-10 pointer-events-none select-none">
            <div className="absolute left-[76%] top-[14%] -translate-x-1/2 text-center text-xs font-black uppercase leading-tight text-primary">
              Positive Direction
            </div>

            <div className="absolute left-[61%] top-[33%] -translate-x-1/2 -translate-y-1/2 text-base font-bold text-foreground">
            ...
            <div className="absolute left-[20%] top-[70%] w-30 -translate-x-1/2 text-center">
              ...
              <div className="mt-1 text-[11px] font-black uppercase leading-tight text-muted-foreground">Origin</div>
            </div>
            ...
            <div className="absolute left-[45%] top-[70%] w-38 -translate-x-1/2 text-center">
              ...
              <div className="mt-1 text-[11px] font-black uppercase leading-tight text-muted-foreground">Initial Position</div>
            </div>
            ...
            <div className="absolute left-[75%] top-[70%] w-38 -translate-x-1/2 text-center">
              ...
              <div className="mt-1 text-[11px] font-black uppercase leading-tight text-muted-foreground">Final Position</div>
            </div>
          </div>
        </div>
