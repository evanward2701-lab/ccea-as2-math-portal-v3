...
export const M1ModellingSandboxSVG: React.FC = () => (
  ...
  >
    <div className="relative w-full aspect-76/41 max-w-4xl mx-auto rounded-xl overflow-hidden border border-zinc-800/60 bg-[#141416] shadow-inner">
      <SVGLibrary />
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 760 410" fill="none" xmlns="http://www.w3.org/2000/svg" shapeRendering="geometricPrecision" role="img" aria-label="Real world towing system simplified into a mathematical model using assumptions">
        {/* Panels */}
        <rect x="24" y="28" width="300" height="244" rx="8" fill="#141417" stroke="#3f3f46" strokeWidth="1.8" />
        <rect x="436" y="28" width="300" height="244" rx="8" fill="#141417" stroke="#3f3f46" strokeWidth="1.8" />

        {/* Real-world towing sketch */}
        <line x1="48" y1="218" x2="300" y2="218" stroke="#64748b" strokeWidth="3" />
        {Array.from({ length: 11 }).map((_, idx) => (
          <line key={idx} x1={58 + idx * 22} y1="218" x2={50 + idx * 22} y2="228" stroke="#3f3f46" strokeWidth="2" />
        ))}
        <rect x="64" y="154" width="82" height="38" rx="5" fill="#1e293b" stroke="#e2e8f0" strokeWidth="2" />
        <path d="M 78 154 L 94 132 H 128 L 142 154" stroke="#e2e8f0" strokeWidth="2" fill="#3f3f46" />
        ...
        <path d="M 210 164 L 224 146 H 248 L 260 164" stroke="#cbd5e1" strokeWidth="2" fill="#3f3f46" />
      ...
      <DiagramLabel position={{ left: '23%', top: '25.9%' }}>
        <div className="grid grid-cols-3 gap-2 text-center text-[11px] font-semibold text-amber-200">
          ...
          <div className="rounded border border-amber-900/60 bg-[#141416]/80 px-2 py-1">air</div>
        </div>
      </DiagramLabel>
      ...
      <DiagramLabel position={{ left: '50%', top: '82.4%' }}>
        <div className="grid grid-cols-2 gap-2 text-center text-xs font-semibold text-zinc-200 md:grid-cols-5 w-full max-w-164">
          ...
          <div className="rounded border border-zinc-900/60 bg-zinc-950/30 px-2 py-2">rigid towbar: does not bend</div>
          <div className="rounded border border-zinc-900/60 bg-zinc-950/30 px-2 py-2">smooth surface: no friction</div>
          <div className="rounded border border-zinc-900/60 bg-zinc-950/30 px-2 py-2">inextensible idea: same acceleration</div>
        </div>
      </DiagramLabel>
    </div>
  </DiagramPanel>
