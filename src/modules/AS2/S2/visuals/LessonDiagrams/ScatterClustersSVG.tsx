    ...
    analysis={
      <div className="mx-auto grid w-full max-w-5xl gap-4 md:grid-cols-[1fr_1fr]">
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm leading-relaxed text-foreground shadow-sm">
          <span className="font-bold text-foreground">Check the structure first:</span> one overall regression line can hide distinct subgroup patterns.
        </div>
        <div className="rounded-xl border border-destructive/30 bg-destructive/20 p-4 text-sm leading-relaxed text-destructive shadow-sm">
          <span className="font-bold text-destructive">CCEA Exam Pitfall:</span> do not blindly trust a single PMCC value or line of best fit. Inspect the scatter first.
        </div>
      </div>
    }
  >
    <div className="mx-auto w-full max-w-5xl rounded-2xl border border-primary/20 bg-card p-5 shadow-2xl">
      <div className="relative aspect-16/9 overflow-hidden rounded-xl border border-primary/20 bg-card shadow-inner">
        ...
        >
          ...
          <rect x="92" y="64" width="792" height="410" rx="26" fill="var(--background)" fillOpacity="0.42" stroke="var(--border)" strokeWidth="1.2" />

          <g opacity="0.22">
            <path d="M140 130 H820" stroke="var(--border)" strokeWidth="1" />
            <path d="M140 230 H820" stroke="var(--border)" strokeWidth="1" />
            <path d="M140 330 H820" stroke="var(--border)" strokeWidth="1" />
            <path d="M140 430 H820" stroke="var(--border)" strokeWidth="1" />
          </g>
          ...
          {groupAPoints.map(([cx, cy], index) => (
            <circle key={`group-a-${index}`} cx={cx} cy={cy} r="9.5" fill="#facc15" stroke="var(--background)" strokeWidth="2" />
          ))}
          {groupBPoints.map(([cx, cy], index) => (
            <circle key={`group-b-${index}`} cx={cx} cy={cy} r="9.5" fill="#c084fc" stroke="var(--background)" strokeWidth="2" />
          ))}
        </svg>

        <DiagramLabel x="16%" y="13.5%" text="y" className="text-lg text-foreground" />
        <DiagramLabel x="88%" y="85%" text="x" className="text-lg text-foreground" />
        ...
        <DiagramLabel x="71%" y="74%" className="text-center">
          <div className="max-w-72 rounded-xl border border-destructive/30 bg-destructive/20 px-4 py-3 text-left shadow-2xl backdrop-blur-sm">
            <div className="mb-1 text-2.5 font-black uppercase tracking-[0.24em] text-destructive">Single Overall Fit</div>
